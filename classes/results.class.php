...
class UCCASS_Results extends UCCASS_Main {
    ...
    function survey_results($sid = 0) {
        ...
        if ($rs === FALSE) {
            $this->error('Error retrieving survey information: ' . $this->db->ErrorMsg());
            return;
        }
        if ($r = $rs->FetchRow($rs)) {
        ...
        if ($rs === FALSE) {
            $this->error('Error getting average, min and max survey times: ' . $this->db->ErrorMsg());
            return;
        }
        ...
        $rs = $this->db->Execute($sql);
        if ($rs === FALSE) {
            $this->error("Error retrieving questions: " . $this->db->ErrorMsg());
            return;
        }
        ...
        $rs = $this->db->Execute($sql);
        if ($rs === FALSE) {
            $this->error("Error retrieving answers: " . $this->db->ErrorMsg());
            return;
        }
        while ($r = $rs->FetchRow($rs)) {
    ...
    function survey_results_text($sid, $qid) {
        ...
        if (!empty($_REQUEST['delete_rid']) && $answer['delete_access']) {
            ...
            if ($rs === FALSE) {
                $this->error('Error deleting checked answers: ' . $this->db->ErrorMsg());
                return;
            }
        }
        ...
        if ($rs === FALSE) {
            return $this->error("Unable to select question: " . $this->db->ErrorMsg());
        }
        if ($r = $rs->FetchRow($rs)) {
    ...
    function filter($sid) {
        ...
        $old_aid = '';
        if ($rs === FALSE) {
            $this->error("Error selecting filter questions: " . $this->db->ErrorMsg());
        }
        if ($r = $rs->FetchRow()) {
            do {
        ...
        if ($rs === FALSE) {
            $this->error("Error selecting min/max survey dates: " . $this->db->ErrorMsg());
        }
        $r = $rs->FetchRow();
    ...
    function process_filter($sid) {
        ...
        if ($num_criteria || $num_dates) {
            ...
            $rs = $this->db->Execute($sql);
            if ($rs === FALSE) {
                return $this->error("Error selecting sequences: " . $this->db->ErrorMsg());
            }
