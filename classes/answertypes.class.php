...
class UCCASS_AnswerTypes extends UCCASS_Main {
    ...
    function edit_answer($sid, $aid) {
        ...
        $rs = $this->db->Execute($query);
        if ($rs === FALSE) {
            $this->error("Error getting survey count: " . $this->db->ErrorMsg());
            return;
        }
        $r = $rs->FetchRow($rs);
        if ($r['c'] > 0) {
        ...
        if (isset($_REQUEST['delete_submit']) && isset($_REQUEST['delete'])) {
            ...
            if ($rs === FALSE) {
                $this->error('Error getting aid values from answer_types table: ' . $this->db->ErrorMsg());
                return;
            } else {
                ...
                if ($rs === FALSE) {
                    $this->error('Error deleting answer values: ' . $this->db->ErrorMsg());
                    return;
                }
            }
            ...
            $rs = $this->db->Execute($query);
            if ($rs === FALSE) {
                $this->error('Error deleting answer types: ' . $this->db->ErrorMsg());
                return;
            }
        ...
        if ($load_answer) {
            ...
            $rs = $this->db->Execute($query);
            if ($rs === FALSE) {
                $this->error("Error selecting answer type information: " . $this->db->ErrorMsg());
                return;
            }
            if ($r = $rs->FetchRow($rs)) {
                $answer = array();
                ...
                $rs = $this->db->Execute($query);
                if ($rs === FALSE) {
                    $this->error('Error getting answer values: ' . $this->db->ErrorMsg());
                    return;
                }
                if ($r = $rs->FetchRow($rs)) {
                    do {
