...
class UCCASS_Config {
    ...
    function load_file($file) {
        ...
        if (file_exists($file)) {
            ...
            if ($fp = fopen($file, "r")) {
                ...
                $ini_file = preg_replace('/^([a-z0-9_.-]+)\s?=\s?"?([^\r\n]+)?"?$/im', "</pre>$1: <input type=\"text\" name=\"$1\" value=\"$2\" size=\"40\"></td></tr><tr><td><pre>\n", $ini_file);
                ...
                $this->error("Cannot read configuration file: $file");
                return;
            }
        } else {
            $this->error("Configuration file does not exist: $file");
            return;
        }
    ...
    function process_config($file) {
        if (file_exists($file)) {
            ...
            $this->error("Config file does not exist: $file");
            return;
        }
