...
class UCCASS_Main {
    ...
    function _CheckLogin($sid = 0, $priv = EDIT_PRIV, $redirect_page = '') {
        ...
        if ((isset($_SESSION['priv'][0][ADMIN_PRIV]) && $_SESSION['priv'][0][ADMIN_PRIV] == 1) || (isset($_SESSION['priv'][$sid][$priv]) && $_SESSION['priv'][$sid][$priv] == 1)) {
            $retval = TRUE;
        } else {
    ...
    function _checkUsernamePassword($sid, $priv, $numallowed = 0, $numseconds = 0) {
        ...
        if (isset($_REQUEST['username']) && isset($_REQUEST['password'])) {
            ...
            $rs = $this->db->Execute($query);
            if ($rs === FALSE) {
                $this->error('Error retrieving user permissions: ' . $this->db->ErrorMsg());
                return FALSE;
            }
            ...
            if ($r = $rs->FetchRow($rs)) {
                ...
                if (!strcmp($password, $r['password'])) {
                    if ($r['admin_priv']) {
                        ...
                        $rs = $this->db->Execute($query);
                        if ($rs === FALSE) {
                            $this->error('Error getting count of completed surveys: ' . $this->db->ErrorMsg());
                            return FALSE;
    ...
    function _checkInvitation($sid, $priv, $numallowed = 0, $numseconds = 0) {
        ...
        if (isset($_REQUEST['invite_code'])) {
            ...
            $rs = $this->db->Execute($query);
            if ($rs === FALSE) {
                $this->error('Error getting permissions while checking invitation code: ' . $this->db->ErrorMsg());
                return FALSE;
            } elseif ($r = $rs->FetchRow($rs)) {
    ...
    function log($message) {
        ...
        if ($rs === FALSE) {
            $this->error('Error Updating history: ' . $this->db->ErrorMsg());
        }
    }
