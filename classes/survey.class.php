...
class UCCASS_Survey extends UCCASS_Main {
    ...
    function take_survey($sid) {
        ...
        if ($rs === FALSE) {
            $this->error("Error retrieving Survey:" . $this->db->ErrorMsg());
            return;
        }
        if ($r = $rs->FetchRow($rs)) {
            if (($r['active'] == 0 || $now < $r['start_date'] || ($now > $r['end_date'] && $r['end_date'] != 0)) && !isset($_SESSION['take_survey']['preview_survey'])) {
                $this->error("Survey #$sid. <em>{$r['name']}</em> in not active at this time");
                return;
            }
        } else {
            $this->error("Survey $sid does not exist or has no questions.");
            return;
        }
        ...
        if ($this->CONF['default_template'] != $survey['template']) {
            if (!$this->set_template_paths($survey['template'])) {
                $this->error("Unable to load template for survey. Expecting to find template in {$this->CONF['template_path']}");
                return;
            }
        }
        ...
        switch ($_SESSION['take_survey']['page']) {
            ...
            case $survey['total_pages'] + 2:
                ...
                if ($rs === FALSE) {
                    $this->error('Error updating elapsed time: ' . $this->db->ErrorMsg());
                }
                unset($_SESSION['take_survey']);
                break;
            ...
            default:
                ...
                if ($rs === FALSE) {
                    $this->error("Error retrieving dependencies: " . $this->db->ErrorMsg());
                    return;
                }
