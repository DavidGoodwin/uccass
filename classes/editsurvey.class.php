...
class UCCASS_EditSurvey extends UCCASS_Main {
    ...
    function _loadProperties($sid) {
        ...
        $rs = $this->db->Execute($query);
        if ($rs === FALSE) {
            $this->error("Error loading Survey #$sid: " . $this->db->ErrorMsg());
            return;
        } elseif ($r = $rs->FetchRow($rs)) {
            ...
            $this->error("Survey #$sid does not exist.");
            return;
        }
    }
    ...
    function _loadQuestions($sid) {
        ...
        $rs = $this->db->Execute($query);
        if ($rs === FALSE) {
            $this->error("Error selecting questions: " . $this->db->ErrorMsg());
            return;
        }
        ...
        if ($r = $rs->FetchRow($rs)) {
            ...
            $rs = $this->db->Execute($query);
            if ($rs === FALSE) {
                $this->error('Error selecting dependencies for survey: ' . $this->db->ErrorMsg());
                return;
            }
        ...
        if ($rs === FALSE) {
            $this->error('Unable to retrieve answer types: ' . $this->db->ErrorMsg());
            return;
        }
        while ($r = $rs->FetchRow($rs)) {
        ...
        if (strlen($_REQUEST['question']) == 0) {
            ...
            if (strcasecmp($_REQUEST['question'], $this->CONF['page_break']) == 0) {
                ...
                if (empty($error)) {
                    ...
                    if (isset($_REQUEST['option'])) {
                        foreach ($_REQUEST['option'] as $num => $option) {
                            if (!empty($option) && !empty($_REQUEST['dep_qid'][$num]) && !empty($_REQUEST['dep_aid'][$num]) && in_array($option, $this->CONF['dependency_modes'])) {
    ...
    function _loadEditQuestion($sid, $qid) {
        ...
        $rs = $this->db->Execute($query);
        if ($rs === FALSE) {
            $this->error('Error selecting data for question: ' . $this->db->ErrorMsg());
            return;
        }
        ...
        if ($rs === FALSE) {
            $this->error('Unable to retrieve answer types from database: ' . $this->db->ErrorMsg());
            return;
        }
        ...
        $rs = $this->db->Execute($query);
        if ($rs === FALSE) {
            $this->error('Unable to retrieve existing dependencies: ' . $this->db->ErrorMsg());
            return;
        }
    ...
    function _loadAccessControl($sid) {
        ...
        $rs = $this->db->Execute($query);
        if ($rs === FALSE) {
            $this->error('Unable to retrieve survey access control information: ' . $this->db->ErrorMsg());
            return;
        } elseif ($r = $rs->FetchRow($rs)) {
            ...
            $this->error('Invalid survey. Survey does not exist.');
            exit();
        }
    }
    ...
    function _loadUsers($sid, $access_control, $date_format) {
        ...
        $rs = $this->db->Execute($query);
        if ($rs === FALSE) {
            $this->error('Error retrieving users: ' . $this->db->ErrorMsg());
            return;
        } elseif ($r = $rs->FetchRow($rs)) {
            do {
    ...
    function _getInviteCode($sid, $uid, $type, $length) {
        ...
        while (!$code && $recursion_level <= $recursion_limit) {
            ...
            if ($code) {
                ...
                $rs = $this->db->Execute($query);
                if ($rs === FALSE) {
                    $this->error('Error checking for duplicate code: ' . $this->db->ErrorMsg());
                    return FALSE;
                } elseif ($r = $rs->FetchRow($rs)) {
        ...
        if ($code) {
            ...
            $rs = $this->db->Execute($query);
            if ($rs === FALSE) {
                $this->error("Error updating invitation code for invitee (uid:$uid): " . $this->db->ErrorMsg());
                return FALSE;
            }
        }
